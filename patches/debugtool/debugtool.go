...
func NewCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "debugtool <namespace> <podname> | debugtool <podname>.<namespace>",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 2 {
				...
				ns = parts[1]
				fmt.Printf(
					"⚠️  Detected pod name with namespace, parsed as namespace=%s, pod=%s\n",
					ns, pod,
				)
			}
